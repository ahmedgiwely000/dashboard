        ...
        <div class="row">
            <div class="col-xl-12 order-xl-1">
                <div class="card bg-secondary shadow">
                    ...
                    <form action="{{route('Quizzes.update',$quiz)}}" method="POST" autocomplete="off" enctype="multipart/form-data">
                        @csrf
                        @method("patch")
                    ...
                    <div class="pl-lg-4">
                        ...
                        <div class="form-group{{ $errors->has('name') ? 'has-danger' : ' '}}">
                            <label for="input-name" class="form-control-label">{{__('Video name')}}</label>
                        <input type="text" name="name" value="{{$quiz->name}}" id="input-name" class="form-control
                        ...
                        <div class="form-group{{ $errors->has('course_id') ? 'has-danger' : ' '}}">
                            ...
                            <select name="course_id" id="input-course_id" required class="form-control">
                                ...
                                <option value="{{$course->id}}" {{ $course->title == $quiz->course->title ? 'selected' : ''}}>{{$course->title}}</option>
                                @endforeach
                            </select>
