...
@section('content')
    @include('admin.users.partials.header', [
        'title' => __('Edit Question') . ' '. auth()->user()->name,
    ])
        ...
        <div class="row">
            <div class="col-xl-12 order-xl-1">
                <div class="card bg-secondary shadow">
                    ...
                    <form action="{{route('Questions.update',$question)}}" method="POST" autocomplete="off" enctype="multipart/form-data">
                        @csrf
                        @method('put')
                    ...
                    <div class="pl-lg-4">
                        ...
                        <input type="text" name="title" value="{{$question->title}}" id="input-title" class="form-control
                        ...
                        required>{{$question->answers}}</textarea>
                        ...
                        <input type="text" name="right_answer" value="{{$question->right_answer}}" id="input-right_answer" class="form-control
                        ...
                        <div class="form-group{{ $errors->has('type') ? 'has-danger' : ' '}}">
                            <label for="input-type" class="form-control-label">{{__('Question Type')}}</label>
                        ...
                        <div class="form-group{{ $errors->has('quiz_id') ? 'has-danger' : ' '}}">
                            ...
                            <option value="{{$quiz->id}}" {{$quiz->name == $question->quiz->name ? 'selected' : ''}}>{{$quiz->name}}</option>
                                @endforeach
                            </select>
